...
void agregarPlanosDeCortePorDesigualdadClique(CPXENVptr env, CPXLPptr lp, double* solucion, Grafo grafo) {
  int cantidadDeNodos = grafo.getCantidadDeNodos();
...
void agregarPlanosDeCorte(CPXENVptr env, CPXLPptr lp, Grafo grafo) {
  ...
  for(int i = 0; i < IT_PLANOS_DE_CORTE; i++) {
    resolverLP(env, lp);
    CPXgetx(env, lp, solucion, 0, cantidadDeVariables - 1);

    agregarPlanosDeCortePorDesigualdadClique(env, lp, solucion, grafo);
...
int main(int argc, char **argv) {
  ...
  agregarPlanosDeCorte(env, lp, grafo);
